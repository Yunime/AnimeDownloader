...
function clear() {
    ...
    if (session > 0) {
        clearDeadSession();
        logger.info("Clear dead sessions...Session cleared: " + (session - getSessionLenght()))
    ...
    if (!object.isNull(token) && token.length == settings.tokenLength) //se il token non è nullo
    {
        for (var c = 0; c < getSessionLenght(); c++) {
            if (dataAuthentications.sessions[c].token == token) {
                ...
                logger.info("Current sessions active: " + getSessionLenght(), "sessionManager.js")
                printActiveSession();
                return id;
            }
        }
...
function renewSessionFn(token) {
    ...
    if (settings.needRenew == true) {
        if (!isTokenExpired(token)) {
            ...
            logger.error("Token invalid or expired", "sessionManager.js");
        }
    }
    else
        return token;
...
function getSessionLenght() {
    ...
    try {
        ...
        //logger.error("Error in function getSessionLenght","sessionManager.js");
    }
    return c;
}
